...
// Used for returnig the wifi configuration
...
/**
 * @brief Wifi application avent group handle and status bist
 * 
 */
		...
		switch (event_id)
		{
			...
			case WIFI_EVENT_STA_DISCONNECTED:
				...
				*wifi_event_sta_disconnected = *((wifi_event_sta_disconnected_t*)event_data);
				printf("WIFI_DISCONNECTED, reason code %d\n", (*wifi_event_sta_disconnected).reason);
		...
		if (xQueueReceive(wifi_app_queue_handle, &msg, portMAX_DELAY))
		{
			switch (msg.msgID)
			{
				...
				case WIFI_APP_MSG_STA_DISCONNECTED:
					...
					if(eventBits & WIFI_APP_CONNECTING_USING_SAVED_CREDS_BIT)
					{
						ESP_LOGI(TAG, "WIFI_APP_CONNECTING_USING_SAVED_CREDENTIALS");
						xEventGroupClearBits(wifi_app_event_group, WIFI_APP_CONNECTING_USING_SAVED_CREDS_BIT);
					}
					...
					{
						ESP_LOGI(TAG, "USER_REQUESTED_DISCONNECTION");
